"""Functions to allow the computer to decide where to play (i.e., 
select the move will maximize utility) based on the current boards
utility.
"""
...
def decideplay(utility, trans_number):
    """Decide move based on the transformed boards utility.
    Returns move on the true and transformed board."""
    ...
    rand_id = np.random.randint(0, len(rows)) if rows.size > 1 else 0
    tran_row = rows[rand_id]
    tran_col = cols[rand_id]
    
    # tran_row and tran_col are locations on the transformed board, and
    # not on the current game board, so we need to transform them back.
    ex = np.zeros((3, 3))
...
class Test_DecidePlay(unittest.TestCase):
    def test_known_move(self):
        ...
        true_move, tran_move = decideplay(game, 0)
    ...
    def test_transform_known_move(self):
        ...
        true_move, tran_move = decideplay(util, 7)
        self.assertTupleEqual(true_move, (2,0))
